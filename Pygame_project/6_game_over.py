        ...
        for weapon_idx, weapon_val in enumerate(weapons):
            ...
            if weapon_rect.colliderect(ball_rect):
                ...
                if ball_img_idx < 3 :
                    ...
                    #왼쪽으로 튕겨가는 작은공
                    balls.append({
                        "pos_x": ball_pos_x +  (ball_width/2) - (small_ball_width /2),    #현재볼의 x 위치
                        "pos_y" : ball_pos_y + (ball_height/2) - (small_ball_height /2),   #현재볼의 y 위치 
                        "img_idx" : ball_img_idx+1,  #현재의 image index
                        "to_x" : -3,   # x direction   #현재볼의 X 방향
                        "to_y" : -6,  # y direction
                        "init_speed_y" : ball_speedn_y[ball_img_idx+1]   #두번째 볼 스피드 
                    })
                    #오른쪽으로 튕겨가는 작은공
                    balls.append({
                        "pos_x": ball_pos_x +  (ball_width/2) - (small_ball_width /2),    #현재볼의 x 위치
